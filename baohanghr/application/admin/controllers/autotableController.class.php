...
class autotableController extends baseController{
	...
	public function updateAction()
	{
	    ...
	    if ($_tableModel->update($data)) {
	        ...
	        $data_return["msg"]="更新失败（数据未修改或数据格式有误）";
	...
	public function xiazaiAction(){
	    $id=$_POST['id'];
        $fu=new yingpinxiaoxiModel();
        $fuList=$fu->getFuJian($id);
        if(empty($fuList['shangchuanfujian'])){
            $data_return["status"]=false;
            $data_return["msg"]="附件不存在";
            $data_return["code"]=1;
            $this->common->ajaxReturn($data_return);exit;
        }else{
            //获取域名
            $url=$_SERVER['SERVER_NAME'];
            $filepath = $url.$fuList['shangchuanfujian'];
            $data_return["status"]=true;
            $data_return["msg"]="附件不存在";
            $data_return["code"]=0;
            $data_return["url"]=$filepath;
            $this->common->ajaxReturn($data_return);exit;
        }
    }
	...
	public function freshAction(){
	    ...
	    $data_return["status"]=true;
	    $data_return["msg"]="刷新成功";
	    $data_return["code"]=0;
	
		$freshlist = $_GET['freshdata'];
		$table_id = $_GET['table_id'];
		if($table_id != 20){
			$data_return["msg"]="请求错误，应聘消息表专用接口";
	        $this->common->ajaxReturn($data_return);
		}

	    $tableModel = new tableModel();
	    $tableName = $tableModel->getTablenameByTableid(20);	 	//应聘消息表专用
		$_tableModel = new Model($tableName);  

	    foreach ($freshlist as $v){
		    if($v=="")
		    {
		        $data_return["msg"]="id 不能为空";
		        $this->common->ajaxReturn($data_return);
			...
			$data = $_tableModel->findOne($v);
		    date_default_timezone_set('PRC');
			$data["fabushijian"] = date('Y-m-d H:i:s', time());//获取当前时间
			$data["neirong"]=html_entity_decode($data["neirong"]);
