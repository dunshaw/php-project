...
class tableController extends baseController{
	...
	public function gettablelistbytableidAction(){
	    ...
	    if($table_id == 20){//招聘信息遵循排序字段、发布时间字段、id字段依次优先的规则；
	    	$_tableList1 = $_tableModel->findBySql("select * from {$tableName} where {$table_sql} ORDER BY paixu+0 desc, fabushijian desc, id desc limit {$page},{$limit} ");
	    }else if($table_id == 21){//应聘信息按照添加时间排序
	    	$_tableList1 = $_tableModel->findBySql("select * from {$tableName} where {$table_sql} ORDER BY dtime desc,id desc limit {$page},{$limit} ");
		}else{
	...
	public function insertAction(){
	    ...
	    $data['u11'] = empty($data['u11'])?"off":"on";
